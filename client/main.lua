...
CreateThread(function()
	while true do
		...
		if dist <= 25.0 then
			...
			if dist <= 2.0 then
				DrawText3D(Config.MissionMarker.x, Config.MissionMarker.y, Config.MissionMarker.z, "druk op ~b~[E]~w~ Om de missie te starten")
...
RegisterNetEvent('AttackTransport:InfoForLspd', function(x, y, z)
	if PlayerJob ~= nil and PlayerJob.name == 'police' then
		if PoliceBlip == 0 then
			...
			AddTextComponentString('Aanval op het transport van contant geld')
...
RegisterNetEvent('qb-armoredtruckheist:client:robberyCall', function(streetLabel, coords)
	if PlayerJob.name == "police" then
		...
		BeginTextCommandSetBlipName('STRING')
		AddTextComponentString("10-90: Gepantserde vrachtwagenoverval")
		EndTextCommandSetBlipName(blip)
		while transG ~= 0 do
			Wait(180 * 4)
			transG = transG - 1
...
function MissionNotification()
	...
	TriggerServerEvent('qb-phone:server:sendNewMail', {
		sender = "De baas",
		subject = "Nieuw doel",
		message = "Dus je bent geïnteresseerd om wat geld te verdienen? Goed... ga een wapen kopen en zorg dat het gebeurt... Ik stuur je nu de locatie.",
	})
end
---
--
	...
	CreateThread(function()
		...
		while not spawned do
			...
			if distance < 200 then
				...
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString('Bestelwagen met contant geld')
	...
	if IsVehicleStopped(transport) then
		if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and GuardsDead == 1 then
			if not IsEntityInWater(PlayerPedId()) then
				...
				hideLastHint()
				QBCore.Functions.Notify('De lading zal tot ontploffing worden gebracht in '..Config.TimeToBlow ..' seconden.', "error")
...
CreateThread(function()
    while true do
		...
		if lootable == 1 then
			...
			if dist <= 4.5 then
				sleep = 0
				hintToDisplay('Druk op [E] om het geld te pakken')
